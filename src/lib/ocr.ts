...
const LAB_DEFS: LabDef[] = [
  { name: "Hemoglobin", keys: /\b(?:hb|h[ae]moglobin)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "g/dL", low: 12, high: 16 },
  { name: "WBC", keys: /\b(?:wbc|tlc|leucocyte)\b[^0-9]{0,16}(\d{3,6}(?:\.\d+)?)/i, unit: "/µL", low: 4000, high: 11000 },
  { name: "Platelets", keys: /\b(?:platelet[s]?|plt)\b[^0-9]{0,16}(\d{4,7})/i, unit: "/µL", low: 150000, high: 450000 },
  { name: "FBS", keys: /\b(?:fbs|fasting(?:\s+blood)?\s+sugar|fasting glucose)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 70, high: 100 },
  { name: "PPBS", keys: /\b(?:ppbs|post[\s-]?prandial)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 70, high: 140 },
  { name: "RBS", keys: /\b(?:rbs|random(?:\s+blood)?\s+sugar)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 70, high: 140 },
  { name: "HbA1c", keys: /\b(?:hba1c|a1c|glycated)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "%", low: 4, high: 5.6 },
  { name: "Creatinine", keys: /\b(?:creatinine|creat\.?)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 0.6, high: 1.2 },
  { name: "Urea", keys: /\b(?:urea|bun)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 15, high: 40 },
  { name: "Sodium", keys: /\b(?:sodium|na\+?)\b[^0-9]{0,8}(\d{2,3}(?:\.\d+)?)/i, unit: "mmol/L", low: 135, high: 145 },
  { name: "Potassium", keys: /\b(?:potassium|k\+?)\b[^0-9]{0,8}(\d+(?:\.\d+)?)/i, unit: "mmol/L", low: 3.5, high: 5.1 },
  { name: "SGPT / ALT", keys: /\b(?:sgpt|alt)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "U/L", low: 7, high: 56 },
  { name: "SGOT / AST", keys: /\b(?:sgot|ast)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "U/L", low: 10, high: 40 },
  { name: "Bilirubin (T)", keys: /\b(?:bilirubin|tbil)\b[^0-9]{0,16}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 0.2, high: 1.2 },
  { name: "TSH", keys: /\b(?:tsh)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "mIU/L", low: 0.4, high: 4.0 },
  { name: "LDL", keys: /\b(?:ldl)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 0, high: 100 },
  { name: "HDL", keys: /\b(?:hdl)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 40, high: 90 },
  { name: "Triglycerides", keys: /\b(?:triglyceride[s]?|tg)\b[^0-9]{0,10}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 0, high: 150 },
  { name: "Total cholesterol", keys: /\b(?:total cholesterol|cholesterol)\b[^0-9]{0,12}(\d+(?:\.\d+)?)/i, unit: "mg/dL", low: 0, high: 200 },
  { name: "ESR", keys: /\b(?:esr)\b[^0-9]{0,8}(\d+(?:\.\d+)?)/i, unit: "mm/hr", low: 0, high: 20 },
  { name: "CRP", keys: /\b(?:crp)\b[^0-9]{0,8}(\d+(?:\.\d+)?)/i, unit: "mg/L", low: 0, high: 5 },
];
...
const DIAG_LINE =
  /\b(type\s*2\s*diabetes|diabetes mellitus|t2dm|hypertension|ihd|cad|copd|asthma|hypothyroidism|ckd|nafld|anemia|anaemia|uti|enteric fever|typhoid|malaria|dengue|osteoarthritis|rheumatoid|gerd|piles|fissure|migraine|tb|tuberculosis)\b/gi;
...
export function extractFromText(raw: string, docType: string): ExtractedDocument {
  ...
  while ((medMatch = medRe.exec(raw)) !== null) {
    ...
    const dose = rest.match(/(\d+\s*(?:mg|mcg|g|iu|units|%))/i)?.[1] ?? "";
    const frequency =
      rest.match(/\b(od|bd|tds|qid|hs|sos|once daily|twice daily|thrice)\b/i)?.[1] ?? "";
    medications.push({
      name,
      dose,
  ...
  return {
    ...
    labs,
    procedures,
    notes: notes || `${docType} uploaded — limited machine-readable text.`,
    confidence,
  };
}
...
Impression: Uncontrolled T2DM, dyslipidaemia, borderline creatinine.
    `,
  },
  {
    id: "rx-medicine",
...
export function summarizeDocuments(
  ...
  const investigationsSummary = [
    ...
    labs.length ? `Investigations (chronological extract): ${labs.join("; ")}.` : "No structured lab values extracted.",
  ]
    .filter(Boolean)
    .join(" ");
  return {
    ...
    medicationsExtracted: uniqueMeds.length
      ? uniqueMeds.join("; ")
      : "No medications extracted from documents.",
  };
}
