  ...
  async function generateSummary() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setGenStatus({
          message:
            "No GEMINI_API_KEY configured — used the built-in local generator. Add GEMINI_API_KEY to use AI.",
          kind: "info",
        });
      } else if (data.aiUsed) {
        setGenStatus({ message: "Summary generated by AI.", kind: "ok" });
      } else {
        setGenStatus({
      ...
      setGenStatus({ message: "Summary generation failed.", kind: "error" });
    } finally {
      setGenBusy(false);
    }
  }
      ...
      <div>
        ...
        {!summary && (
          <div className="mt-8 space-y-4">
            ...
            <div className="rounded-[28px] bg-[#fffdf7] p-6 ring-1 ring-[#1b1712]/8">
              <p className="text-xs uppercase tracking-[0.16em] text-[#c9842a]">Interview answers (raw)</p>
              <div className="mt-4 space-y-3">
                {bundle.answers.length === 0 && (
                  <p className="text-sm text-[#4a4338]">No answers recorded for this session.</p>
                )}
                {bundle.answers.map((a) => (
                  <div key={a.id} className="border-l-2 border-[#c9842a]/40 pl-3">
                    <p className="text-xs uppercase tracking-wider text-[#0f5c61]">
